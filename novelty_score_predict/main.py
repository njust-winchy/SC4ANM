...
def main(args):
    ...
    model = MyModel_Pool(pretrained_model_name_or_path=args.pretrained_model_name_or_path,
                             num_classes=args.num_classes, config=config)
    model.to(args.device)
    # num_workers
    num_workers = min([os.cpu_count(), args.train_batch_size if args.train_batch_size > 1 else 0, 8])

    # dataset, dataloader
    train_set = MyDataset(train_data, tokenizer, config, args.text_input)
    ...
    valid_set = MyDataset(valid_data, tokenizer, config, args.text_input)
    ...
    log_dir = os.path.join(sys.path[0], "runs", "{}_{}".format(args.text_input, current_time))
    ...
    for epoch in range(args.num_train_epochs):
        train_result = train_one_epoch(model=model,
                                       ...
                                       epoch=epoch,
                                       accmulation_steps=args.accmulation_steps)
        ...
        if dev_result['macro_f1'] > best_macro_f1:
            torch.save(model.state_dict(), os.path.join(args.weights_dir, '{}-{}-epoch{}-macro_f1{:.3f}.pth'.format(
                args.text_input, current_time, epoch, dev_result['macro_f1'])))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--accmulation_steps', type=int, default=params.accmulation_steps)
