...
def train_one_epoch(model, device, data_loader, epoch, optimizer, lr_scheduler, accmulation_steps):
    model.train()
    ...
    for step, data in enumerate(data_loader):
        ...
        loss = loss_function(pred_logits, label.to(device)) / accmulation_steps
        loss.backward()
...
def init_logger(args, current_time):
    ...
    log_path = os.path.join(sys.path[0], "logs", "{}_{}.txt".format('m+r', current_time))
